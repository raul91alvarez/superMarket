import { LogoutComponent } from './addons/components/logout/logout.component';
import { AuthGuardGuard } from './addons/modules/auth-guard/auth-guard.guard';
...
const routes: Routes = [
  ...
  },
  {
    ...
    children: [
      {
        path: '',
        redirectTo: '/store/landingPage',
        pathMatch: 'full',
      },
      {
        path: 'store',
        loadChildren: () => import('./addons/components/store/store.module').then(module => module.StoreModule)
      },
      {
        path: 'prod', canActivate: [AuthGuardGuard],
        loadChildren: () => import('./addons/components/product/product.module').then(module => module.ProductModule)
      },
      // {
      //   path: 'layout',
